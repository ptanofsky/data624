...
fc_atm1_final <- fit_atm1_bc %>% forecast(h = "47 days")
...
atm1_forecast_may2010 <- fc_atm1_final %>%
  filter(.model == "Arima", DATE > "2010-04-30")
...
According to the RMSE, the ETS Additive model produces the most accurate model against the training data at 318.9662.
...
Now forecasting the models on the test dataset, the ARIMA model performs the best with an RMSE of 318.8746. 
...
The above plot indicates the models are not following the weekly seasonal pattern. Let me try the truncated training and test approach similar to ATM1 and ATM2.
...
The ARIMA attains the best AICc value of 841.4916 compared to the ETS models.
...
Forecasting the models for the final 16 days of the dataset, the ETS Multiplicative model achieves the best RMSE at 241.8786.
...
The Box-Cox transformation of the final few months of the dataset helps clarify the weekly seasonal pattern.
...
The ARIMA model with transformation performs the best on the training data with an AICc of 360.7386.

```{r warning=F, message=F}
fit_atm4_bc %>% accuracy()
```

The best accuracy on the training set goes to the ARIMA model with an RMSE of 293.0240.
...
The forecast of the models for the final 16 days of the dataset shows the ARIMA model with the best-performing RMSE of 320.7253.
...
The `gg_subseries` plot indicates increasing usage for almost every month over the course of the dataset. This increase is another indicator the Box-Cox transformation will be needed.
...
The Box-Cox transformation does appear to address the increasing variance in the initial plot.
...
The best performing model according to the AICc is the ARIMA model at -785.4651.
...
The best RMSE on the test data is the Seasonal Naive model followed closely by the ARIMA model and the ETS Multiplicative model.
...
The above plot of the forecasts overlayed with the actual data indicates several of the models are able to predict the monthly seasonal pattern.

### Forecast

Let's forecast for the year 2014 with all the models.
...
Even though the Seasonal Naive model performed the best on the Go with the ARIMA model.

Maybe just go with Seasona Naive

```{r warning=F, message=F}
# Write to excel
# dataframe for Excel output

## TODO: FIX BELOW WITH THE CORRECT OBJECTS
atm4_forecast_may2010 <- fc_atm4_final %>%
  filter(.model == "ETS_Mult", DATE > "2010-04-30")
