...
The plot provides clarity regarding the impact of a given variable. The `AlchRel` variable box indicates a positive relationship with the dependent variable `PH`. The most important variable `MnfFlow` box defines a negative coefficient, and thus an inverse effect on `PH`.
...
```{r}
length(linearTune$finalModel$fitted.values)
#2055
```
...
lm_train_preds <- as.data.frame(cbind(obs=sdata_train_pp$PH, pred=pred_lm_train, model=model_l, dataType=dt_l, object=obj_l))
...
#all_preds <- all_preds %>% `rownames<-`( NULL )
